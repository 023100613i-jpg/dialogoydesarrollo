...
try {
    ...
    $totalPublicadas = $pdo->query("SELECT COUNT(*) FROM noticias WHERE estado = 'publicado'")->fetchColumn();
    $totalArchivadas = $pdo->query("SELECT COUNT(*) FROM noticias WHERE estado = 'archivado'")->fetchColumn();
    $totalBorradores = $pdo->query("SELECT COUNT(*) FROM noticias WHERE estado = 'borrador'")->fetchColumn();
} catch(PDOException $e) {
    $totalPublicadas = $totalNoticias;
}

// Consulta según filtro
$where = '';
if ($filtro === 'publicadas') $where = "WHERE n.estado = 'publicado'";
elseif ($filtro === 'archivadas') $where = "WHERE n.estado = 'archivado'";
elseif ($filtro === 'borradores') $where = "WHERE n.estado = 'borrador'";
...
<head>
    ...
    <title>Noticias · DDP Noticias</title>
...
<body>
    ...
    <div class="main-content">
        <header class="topbar">
            <div class="topbar-left">
                ...
                <div class="breadcrumb">
                    <i class="fas fa-home"></i> / <span>Noticias</span>
                </div>
            </div>
        ...
        <div class="content-area">
            <section class="hero">
                <div>
                    <h1>Gestión de <span>Noticias</span></h1>
                    <p>Administra las noticias que aparecen en la sección Actualidad</p>
                </div>
            ...
            <!-- STATS CLICKEABLES -->
            <div class="stats-row">
                <a href="?filtro=todas" class="stat-card <?php echo $filtro === 'todas' ? 'active' : ''; ?>">
                    <div class="number"><?php echo $totalNoticias; ?></div>
                    <div class="label">Total Noticias</div>
                </a>
                <a href="?filtro=publicadas" class="stat-card <?php echo $filtro === 'publicadas' ? 'active' : ''; ?>">
                    <div class="number"><?php echo $totalPublicadas; ?></div>
                    <div class="label">Publicadas</div>
                </a>
                <a href="?filtro=archivadas" class="stat-card <?php echo $filtro === 'archivadas' ? 'active' : ''; ?>">
                    <div class="number"><?php echo $totalArchivadas; ?></div>
            ...
            <div class="alert alert-success">
                ...
                <?php 
                    $mensajes = [
                        'creado' => 'Noticia creada exitosamente.',
                        'actualizado' => 'Noticia actualizada exitosamente.',
                        'eliminado' => 'Noticia eliminada exitosamente.',
                        'publicado' => 'Noticia publicada exitosamente.',
                        'archivado' => 'Noticia archivada exitosamente.',
                        'borrador' => 'Noticia marcada como borrador.',
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php foreach ($noticias as $noticia): 
                            $estadoActual = $noticia['estado'] ?? 'publicado';
                        ?>
                        <tr>
                            ...
                            <td>
                                <strong><?php echo htmlspecialchars($noticia['titulo'] ?? 'Sin título'); ?></strong>
                            </td>
                            <td>
                            ...
                            <td>
                                <?php if (!empty($noticia['link_externo'])): ?>
                                    <a href="<?php echo htmlspecialchars($noticia['link_externo']); ?>" 
                                       target="_blank" 
                                       style="color:var(--ddp-red);font-size:0.8rem;">
                            ...
                            <td><?php echo isset($noticia['fecha_publicacion']) ? date('d/m/Y', strtotime($noticia['fecha_publicacion'])) : '-'; ?></td>
                            <td>
                                <div class="actions">
                                    <a href="editar.php?id=<?php echo $noticia['id']; ?>" class="edit" title="Editar">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    
                                    <?php if ($estadoActual !== 'publicado'): ?>
                                    <a href="cambiar-estado.php?id=<?php echo $noticia['id']; ?>&estado=publicado" 
                                       class="publish" title="Publicar"
                                       onclick="return confirm('¿Publicar esta noticia?')">
                                        <i class="fas fa-check"></i>
                                    </a>
                                    <?php endif; ?>
                                    
                                    <?php if ($estadoActual !== 'borrador'): ?>
                                    <a href="cambiar-estado.php?id=<?php echo $noticia['id']; ?>&estado=borrador" 
                                       class="draft" title="Marcar como Borrador"
                                       onclick="return confirm('¿Marcar esta noticia como borrador?')">
                                        <i class="fas fa-file-alt"></i>
                                    </a>
                                    <?php endif; ?>
                                    
                                    <?php if ($estadoActual !== 'archivado'): ?>
                                    <a href="cambiar-estado.php?id=<?php echo $noticia['id']; ?>&estado=archivado" 
                                       class="archive" title="Archivar"
                                       onclick="return confirm('¿Archivar esta noticia?')">
            ...
            <div class="empty-state">
                ...
                <h3>No hay noticias<?php echo $filtro !== 'todas' ? ' en este filtro' : ''; ?></h3>
                <p>
                    <?php if ($filtro === 'todas'): ?>
                        Comienza a gestionar las noticias que aparecerán en la sección Actualidad.
                    <?php else: ?>
                        No hay noticias con el estado seleccionado.
                    <?php endif; ?>
                </p>
                <?php if ($filtro === 'todas'): ?>
                <a href="crear.php" class="btn btn-red">
