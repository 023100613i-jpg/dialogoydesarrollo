...
if ($filtro === 'publicados') {
    $whereFiltro = "WHERE r.estado = 'publicado'";
} elseif ($filtro === 'borradores') {
    $whereFiltro = "WHERE r.estado = 'borrador'";
}
// si es 'todos' no se filtra

// Contadores
$totalTodos = $pdo->query("SELECT COUNT(*) FROM reportajes")->fetchColumn();
$totalPublicados = $pdo->query("SELECT COUNT(*) FROM reportajes WHERE estado = 'publicado'")->fetchColumn();
$totalBorradores = $pdo->query("SELECT COUNT(*) FROM reportajes WHERE estado = 'borrador'")->fetchColumn();

// Consulta de reportajes con filtro
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sidebar {
            ...
            background: var(--ddp-white); border-right: 1px solid var(--ddp-border);
...
@media (max-width: 992px) {
    .topbar-right .user-menu .user-info { display: none; }
        ...
        .filtro-btn.publicados {
            background: transparent;
            color: var(--ddp-text-secondary);
        }
        .filtro-btn.publicados:hover {
            background: var(--ddp-bg);
        }
        .filtro-btn.publicados.active {
            background: #d4edda;
            color: var(--ddp-success);
        }
        .filtro-btn.borradores {
            background: transparent;
            color: var(--ddp-text-secondary);
        }
        .filtro-btn.borradores:hover {
            background: var(--ddp-bg);
        }
        .filtro-btn.borradores.active {
            background: #fff3cd;
        ...
        .table thead th {
            ...
            color: var(--ddp-text-secondary); font-weight: 600;
        ...
        .table .thumb { width: 60px; height: 40px; object-fit: cover; border-radius: 4px; border: 1px solid var(--ddp-border); }
        ...
        .empty-state { text-align: center; padding: 4rem 2rem; background: var(--ddp-white); border-radius: var(--ddp-radius); border: 1px solid var(--ddp-border); }
        .empty-state .icon-wrapper { width: 80px; height: 80px; margin: 0 auto 1.5rem; background: var(--ddp-red-light); border-radius: 50%; display: flex; align-items: center; justify-content: center; }
...
<body>
        ...
        <header class="topbar">
            <div class="topbar-left">
                ...
                <div class="breadcrumb">
                    <i class="fas fa-home"></i> / <span>Reportajes</span>
                </div>
            </div>
    ...
    <div class="user-menu" id="userMenu">
        ...
        <div class="content-area">
            <section class="hero">
                <div>
                    <h1>Gestión de <span>Reportajes</span></h1>
                    <p>Administra los reportajes del sitio</p>
                </div>
            ...
            <div class="alert alert-success">
                ...
                <?php 
                    $mensajes = [
                        'creado' => 'Reportaje creado exitosamente.',
                        'actualizado' => 'Reportaje actualizado exitosamente.',
                        'eliminado' => 'Reportaje eliminado exitosamente.'
                    ];
            ...
            <div class="filtros">
                <a href="?filtro=todos" class="filtro-btn todos <?php echo $filtro === 'todos' ? 'active' : ''; ?>">
                    Todos <span class="count"><?php echo $totalTodos; ?></span>
                </a>
                <a href="?filtro=publicados" class="filtro-btn publicados <?php echo $filtro === 'publicados' ? 'active' : ''; ?>">
                    Publicados <span class="count"><?php echo $totalPublicados; ?></span>
                </a>
                <a href="?filtro=borradores" class="filtro-btn borradores <?php echo $filtro === 'borradores' ? 'active' : ''; ?>">
                    Borradores <span class="count"><?php echo $totalBorradores; ?></span>
                </a>
            </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <strong><?php echo htmlspecialchars($reportaje['titulo'] ?? 'Sin título'); ?></strong>
                            </td>
                            <td>
                                <?php if (($reportaje['estado'] ?? 'publicado') === 'publicado'): ?>
                            ...
                            <td><?php echo isset($reportaje['fecha_publicacion']) ? date('d/m/Y', strtotime($reportaje['fecha_publicacion'])) : '-'; ?></td>
                            <td>
                                <div class="actions">
                                    <a href="editar.php?id=<?php echo $reportaje['id']; ?>" class="edit" title="Editar">
            ...
            <div class="empty-state">
                ...
                <h3>No hay reportajes <?php echo $filtro !== 'todos' ? 'en este filtro' : ''; ?></h3>
                <p>Comienza a gestionar los reportajes del sitio.</p>
                <a href="crear.php" class="btn btn-red">
                    <i class="fas fa-plus"></i> Crear primer reportaje
                </a>
            </div>
            <?php endif; ?>
        </div>
